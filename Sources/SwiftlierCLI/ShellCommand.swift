...
public final class ShellCommand: CustomStringConvertible {
    ...
    public func execute() throws -> String {
        ...
        guard self.process.terminationStatus == 0 else {
            ...
            throw GenericSwiftlierError("executing command", because: "it returned a bad response code \(self.process.terminationStatus). The command was '\(command)'.")
        }
...
private extension ShellCommand {
    ...
    func removeFromCommandsToKill() {
        if let index = commandsToKill.firstIndex(where: {$0 === self}) {
